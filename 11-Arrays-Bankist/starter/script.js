...
So each of them asked 5 dog owners about their dog's age, and stored the data into an array 
(one array for each). 
...
years old (which is the same as keeping dogs that are at least 18 years old)
...
const calcAverageHumanAge = function(arrayDogAge){
  const humanAgeArray = arrayDogAge.map(age=>{
    if(age <= 2){
      return 2 * age;
    }else{
      return 16 + age *4;
  ...
  const olderDogs = humanAgeArray.filter(function (mov, i, arr) {
    return mov >= 18;
  });

  const averageHumanAge = olderDogs.reduce((acc,cur)=>{
    return acc + cur;
  },0) / olderDogs.length;
...
const createMovements = function (movements) {
